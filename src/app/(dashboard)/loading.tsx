...
export default function Loading() {
  return (
    <div className="flex flex-col gap-5 px-4 pt-5">
      ...
      {/* Calorie ring */}
      <div className="rounded-3xl p-6 flex flex-col items-center gap-4" style={{ backgroundColor: '#1A1A1A', border: '1px solid #2E2E2E' }}>
        <Skel w="w-44" h="h-44" rounded="rounded-full" />
        <Skel w="w-32" h="h-4" />
      </div>

      {/* Macro bars */}
      <div className="rounded-3xl px-5 py-4 flex flex-col gap-4" style={{ backgroundColor: '#1A1A1A', border: '1px solid #2E2E2E' }}>
        <Skel w="w-36" h="h-4" />
        {[1, 2, 3, 4].map((i) => (
