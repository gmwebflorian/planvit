  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: '#0F0F0F' }}>
      ...
      <div className="px-4 pb-3">
        <div className="h-12 rounded-2xl animate-pulse" style={{ backgroundColor: '#1A1A1A', border: '1px solid #2E2E2E' }} />
      </div>

      {/* Food cards */}
      <div className="px-4 flex flex-col gap-2">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="rounded-2xl px-4 py-3 flex flex-col gap-2" style={{ backgroundColor: '#1A1A1A', border: '1px solid #2E2E2E' }}>
            <Skel w="w-40" h="h-4" />
            <Skel w="w-56" h="h-3" />
          </div>
        ))}
